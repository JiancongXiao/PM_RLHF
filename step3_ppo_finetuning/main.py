...
def parse_args():
    ...
    parser.add_argument("--algo", type=str, default="ppo", help="Algorithm name")
    parser.add_argument(
        "--data_path",
        nargs="*",
    ...
    parser.add_argument(
        "--alpha_ctl", type=float, default=0.0, help="Distribution support coefficient."
    )
    parser.add_argument(
...
def evaluation_ppl(trainer, eval_dataloader, device, max_eval_samples=1000):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        with torch.no_grad():
            outputs = trainer.actor_model(**batch)
...
def evaluation_pm(
    ...
    for step, batch_prompt in enumerate(eval_dataloader):
        ...
        exp2 = trainer.generate_experience(
            batch_prompt["prompt"],
            batch_prompt["prompt_att_mask"],
            step,
            deterministic=False,
        ...
        rewards_diff = exp["rewards"] - exp2["rewards"]

        prompt_length = trainer.prompt_length
        start = prompt_length - 1
        action_mask = exp["attention_mask"][:, 1:]
        action_mask2 = exp2["attention_mask"][:, 1:]

        rewards_diff_log_pi = (exp["logprobs"][:, start:] * action_mask[:, start:]).sum(
            dim=-1
        ) - (exp2["logprobs"][:, start:] * action_mask2[:, start:]).sum(dim=-1)

        pm = torch.mean((rewards_diff - rewards_diff_log_pi) ** 2)
        eval_pm.append(pm.item())

        num_samples += len(exp["rewards"]) * torch.distributed.get_world_size()
        if num_samples >= max_eval_samples // 2:
            break

    return np.mean(eval_pm)
...
def save_model(args, rlhf_engine, tokenizer):
...
def main():
    ...
    pm = evaluation_pm(
        trainer, prompt_eval_dataloader, device, args, args.eval_samples
    )
    print_rank_0(
        f"eval reward: {eval_reward:.2f} | eval length: {eval_length:.0f} | eval kl: {eval_kl:.2f} | eval entropy: {eval_entropy:.2f} | eval ppl: {perplexity:.2f} | eval pm: {pm:.4f}",
        args.global_rank,
    )
    if args.enable_tensorboard and torch.distributed.get_rank() == 0:
        ...
        writer.add_scalar("eval/pm", pm, global_step=global_step)
        writer.flush()
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            if exp_dataset is not None:
                ...
                average_reward, average_return, average_advantage = 0, 0, 0
                ...
                for ppo_ep in range(args.ppo_epochs):
                    for i, (exp_data, unsup_data) in enumerate(
                        ...
                        critic_loss_sum += critic_loss.mean()
                        average_reward += exp_data["rewards"].mean()
                        average_return += returns.mean()
                        average_advantage += advantages.mean()
                ...
                print_rank_0(
                    f"Epoch: {epoch + 1}/{args.num_train_epochs} | Step: {step}/{len(prompt_train_dataloader)} | PPO Epoch: {ppo_ep + 1} | Actor Loss: {actor_loss_sum / inner_iter:.4f} | Critic Loss: {critic_loss_sum / inner_iter:.4f} | Unsupervised Loss: {unsup_loss_sum / inner_iter:.4f}",
                    args.global_rank,
                )
                ...
                average_advantage = (
                    get_all_reduce_mean(average_advantage).item() / inner_iter
                )
                average_actor_loss = (
                    get_all_reduce_mean(actor_loss_sum).item() / inner_iter
                )
                average_critic_loss = (
                    get_all_reduce_mean(critic_loss_sum).item() / inner_iter
                )
                average_unsup_loss = (
                ...
                print_rank_0(
                    f"Reward score: {average_reward:.2f} Return: {average_return:.2f} Length: {average_length:.0f} KL: {average_kl:.2f} Entropy: {average_entropy:.2f} Advantage: {average_advantage:.4f} Grad norm: {actor_grad_norm:.4f}",
                    args.global_rank,
                )
                print_rank_0(
                    "-------------------------------------------------------------------------------------",
                ...
                if args.enable_tensorboard and torch.distributed.get_rank() == 0:
                    ...
                    writer.add_scalar(
                        "train/critic_loss", average_critic_loss, global_step=step
                    )
                    writer.add_scalar(
                    ...
                    writer.add_scalar(
                        "train/advantage", average_advantage, global_step=global_step
                    )
                    writer.add_scalar(
            ...
            if (global_step + 1) % (
                len(prompt_train_dataloader) // args.eval_interval
            ) == 0:
                print_rank_0(
                ...
                if args.enable_tensorboard and torch.distributed.get_rank() == 0:
                    ...
                    writer.add_scalar("eval/ppl", perplexity, global_step=global_step)
                    writer.add_scalar(
                        "eval/pm", pm, global_step=global_step
                    )
                    writer.flush()
