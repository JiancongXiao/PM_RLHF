...
def main():
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            # For the current prompt, generate the first output
            out_1 = trainer.generate_experience(batch_prompt['prompt'],
                                              batch_prompt['prompt_att_mask'],
                                              step)
            exp_dataset_1 = exp_mini_dataset.add(out_1)

            # For the current prompt, generate the second output
            out_2 = trainer.generate_experience(batch_prompt['prompt'],
                                              batch_prompt['prompt_att_mask'],
                                              step)
            exp_dataset_2 = exp_mini_dataset.add(out_2)
