...
class DeepSpeedPPOTrainer:
    def __init__(self, rlhf_engine, args):
        ...
        self.kl_ctl = args.kl_ctl  # 0.1
        self.clip_reward_value = args.clip_reward_value  # 5.0
        self.alpha_ctl = args.alpha_ctl  # 0.0
        self.cliprange = 0.2
    ...
    def _generate_sequence(
        self, prompts, mask, step, deterministic=False, print_answers=True
    ):
        ...
        if print_answers:
            print(
                f"[step={step}, rank={torch.distributed.get_rank()}] {self.tokenizer.batch_decode(seq, skip_special_tokens=True)[0]}"
            )
    ...
    def generate_experience(
        ...
        if self.actor_model._total_batch_size is None:
            bsz = prompts.shape[0]
        ...
        seq = self._generate_sequence(prompts, mask, step, deterministic, print_answers)
        generate_end = time.time()
        if seq is None:
            assert (
                self.last_generated_experience is not None
        ...
        return {
            "prompts": prompts,
            "logprobs": logprobs,  # gather_log_probs(logits[:, :-1, :], seq[:, 1:]),
            "ref_logprobs": ref_logprobs,  # gather_log_probs(logits_ref[:, :-1, :], seq[:,1:]),
            "value": values,
    ...
    def compute_rewards(
        ...
        if self.alpha_ctl > 0:
            condition = ref_log_probs > torch.log(self.alpha_ctl)
            penalty = torch.where(condition, log_probs, log_probs - ref_log_probs)
        else:
            penalty = log_probs - ref_log_probs
        kl_divergence_estimate = -self.kl_ctl * penalty
    ...
    def train_rlhf(self, inputs):
        ...
        return actor_loss, critic_loss, returns, advantages
